  ...
  return (
    <div
      ...
      style={{
        ...
        padding: "clamp(80px, 10vh, 120px) clamp(16px, 2vw, 32px) clamp(16px, 2vw, 32px)",
      }}
    >
      <div
...
function ItemMeta({ item }: { item: LessonItem }) {
  ...
  if (item.duration_minutes)
    chips.push({
      ...
      value: t("minutesShort", { count: item.duration_minutes }),
    });
  if (item.created_at)
    chips.push({ label: t("createdLabel"), value: fmt(item.created_at, locale) });
  if (item.updated_at)
    chips.push({ label: t("updatedLabel"), value: fmt(item.updated_at, locale) });
  ...
  return (
    <div
      style={{
        ...
        paddingBottom: "clamp(12px, 0.83vw, 16px)",
    ...
    >
      {chips.map(({ label, value }) => (
        <span
          ...
          style={{
            fontFamily: F,
            fontSize: "clamp(11px, 0.73vw, 13px)",
    ...
    <Modal onClose={onClose}>
      ...
      <div
        style={{
          display: "flex",
          gap: 8,
          marginBottom: test.description
            ? "clamp(10px, 0.69vw, 12px)"
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        padding: "8px 14px",
                        borderRadius: 10,
                        background: correct ? "rgba(28,187,67,0.08)" : "var(--color-input-bg)",
                        border: `1px solid ${correct ? "var(--color-success)" : "transparent"}`,
                        fontFamily: F,
                        fontSize: "clamp(12px, 0.83vw, 15px)",
                        color: correct ? "var(--color-success)" : "var(--color-text-primary)",
                        transition: "all 0.2s",
                      }}
                    >
                      <span>{opt}</span>
                      {correct && (
                        <span style={{ fontWeight: 700, fontSize: "clamp(10px, 0.63vw, 12px)" }}>
                          ✓ {t("correct")}
                        </span>
                      )}
                    </div>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        fontWeight: 500,
                        fontSize: "clamp(12px, 0.83vw, 15px)",
                        color: correct ? "var(--color-success)" : "var(--color-text-secondary)",
                        transition: "all 0.2s",
                      }}
                    >
            ...
            {q.question_type === "open" && revealed && q.sample_answer && (
              <div
                style={{
                  padding: "10px 14px",
                  borderRadius: 10,
                  background: "rgba(28,187,67,0.07)",
      ...
      <div
        style={{
          marginTop: "clamp(20px, 1.67vw, 28px)",
          display: "flex",
      ...
      >
        <button
          ...
          style={{
            ...
            fontWeight: 600,
            fontSize: "clamp(13px, 0.83vw, 15px)",
            padding: "clamp(8px, 0.63vw, 12px) clamp(18px, 1.46vw, 24px)",
            borderRadius: 999,
            cursor: "pointer",
            transition: "all 0.15s",
            background: revealed ? "var(--color-text-primary)" : "var(--color-bg)",
            color: revealed ? "white" : "var(--color-text-primary)",
...
function ItemRow({ item, onOpen }: { item: LessonItem; onOpen: () => void }) {
  ...
  const title =
    item.item_type === "test"
      ? (item.test?.title ?? t("testFallback"))
      : item.item_type === "video"
        ? (item.original_video_name ?? t("video"))
        : t("textContent");
  ...
  return (
    <button
      ...
      style={{
        ...
        gap: 10,
        width: "100%",
        padding: "clamp(7px, 0.52vw, 10px) clamp(20px, 1.67vw, 28px)",
        background: hovered ? "var(--color-input-bg)" : "none",
        border: "none",
        borderRadius: 10,
        cursor: "pointer",
...
function MaterialsSection({
  ...
  async function handleUpload(files: File[]) {
    ...
    try {
      const uploaded = await Promise.all(
        files.map((f) => uploadLessonDocument(slug, moduleId, lessonId, f)),
      );
  ...
  return (
    <div
      style={{
        paddingLeft: "clamp(20px, 1.67vw, 24px)",
        paddingTop: 6,
        paddingBottom: 4,
        display: "flex",
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          padding: "0 clamp(10px, 0.83vw, 14px)",
        }}
      >
        <Upload size={14} />
...
function LessonAccordion({
  ...
  const [items, setItems] = useState<LessonItem[] | null>(
    lesson.items !== undefined ? lesson.items : null,
  );
  ...
  async function handleMandatoryChange() {
    ...
    try {
      const updated = await updateLesson(slug, moduleId, lesson.id, {
    ...
    <div>
      <div
        style={{
          ...
          gap: "4px 12px",
          width: "100%",
          padding: "clamp(4px, 0.28vw, 5px) 0",
        }}
      >
        <button
          type="button"
        ...
        <label
          ...
          style={{
            ...
            gap: 6,
            flexShrink: 0,
            cursor: savingMandatory ? "default" : "pointer",
            opacity: savingMandatory ? 0.6 : 1,
          }}
        >
          <input
            type="checkbox"
  ...
  return (
    <div
      style={{
        ...
        paddingBlock: "clamp(16px, 1.39vw, 24px)",
      }}
    >
      <button
        type="button"
      ...
      >
        <span
          style={{
            ...
            fontWeight: 600,
            fontSize: "clamp(14px, 1.39vw, 20px)",
            lineHeight: "clamp(22px, 1.74vw, 25px)",
            color: "var(--color-blue)",
            flexShrink: 0,
      ...
      {isOpen && (
        <div
          style={{
            marginTop: "clamp(12px, 1.39vw, 20px)",
            display: "flex",
            flexDirection: "column",
            gap: 4,
          }}
    ...
    <SectionCard>
      ...
      >
        ...
        {allIds.length > 0 && (
          <GradientButton
            type="button"
            onClick={() => setOpenModules(allOpen ? new Set() : new Set(allIds))}
          >
      ...
      {modal?.kind === "test" && (
        <TestModal item={modal.item} test={modal.test} onClose={() => setModal(null)} />
      )}
    </SectionCard>
  );
}
