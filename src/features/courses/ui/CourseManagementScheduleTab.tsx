...
import { ChevronDown, Pencil, Trash2, Check, User, AlertCircle, UserPlus, X } from "lucide-react";
...
export function TimePicker({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  function pick(newH: number, newM: number) {
    onChange(`${padTwo(newH)}:${padTwo(newM)}`);
  }
...
function TimeSlotRow({
  ...
  const handleAssign = async (enrollmentId: number) => {
    ...
    setPickOpen(false);
    try {
      await onAssign(slot.id, enrollmentId);
    } finally {
      setAssigning(false);
    }
  };
...
function DayGroup({
  ...
  onRescheduleConfirm: (
    id: number,
    p: { day_of_week: DayOfWeek; start_time: string; end_time: string },
  ) => Promise<void>;
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span
            style={{
              fontFamily: "var(--font-base)",
              fontWeight: 700,
              fontSize: "clamp(13px, 0.87vw, 15px)",
              color: "var(--color-text-primary)",
            }}
          >
            {tDays(DAY_KEYS[day])}
          ...
          >
            {booked > 0 ? `${t("bookedCount", { count: booked })} · ` : ""}
...
function GenerateSlotsForm({
  ...
  const t = useTranslations("CourseManagementScheduleTab");
  const days = useDayOptions();
  const durationOptions = DURATION_VALUES.map((m) => ({
    value: m,
    label: t("minutesShort", { count: m }),
  }));
  const breakOptions = BREAK_VALUES.map((m) => ({
    value: m,
    label: m === 0 ? t("noBreak") : t("minutesShort", { count: m }),
  }));
  const [day, setDay] = useState<DayOfWeek>(0);
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      await onGenerate(
        preview.map((s) => ({ day_of_week: day, start_time: s.start, end_time: s.end })),
      );
    } catch (e: unknown) {
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        ...
        <div>
          ...
          <PillSelect
            ...
            value={breakMin}
            onChange={(v) => setBreakMin(v as number)}
          />
        </div>
      </div>
...
function IndividualFormatSection({
  ...
  const [conflictRescheduleState, setConflictRescheduleState] = useState<{
    eventId: number;
    date: string;
    start: string;
    end: string;
  ...
  const [reschedulePendingEvents, setReschedulePendingEvents] = useState<
    ScheduleConflictPersonalEvent[]
  >([]);
  const [rescheduleModalSaving, setRescheduleModalSaving] = useState(false);
  const [rescheduleEventReschedule, setRescheduleEventReschedule] = useState<{
    eventId: number;
    date: string;
    start: string;
    end: string;
  ...
  const [assignConflictRescheduleErr, setAssignConflictRescheduleErr] =
    useState<RescheduleErr | null>(null);

  useEffect(() => {
    getScheduleSlots(slug, fmt.id)
      .then(setSlots)
      .finally(() => setLoading(false));
    getCourseEnrolledStudents(slug, fmt.id)
      .then(setEnrolledStudents)
      .catch(() => {});
  }, [slug, fmt.id]);

  const resolveAndCreateSlots = async (payloads: ScheduleSlotPayload[]) => {
    const combined: ScheduleConflicts = {
      group: [],
      individual: [],
      personal: [],
      personal_events: [],
    ...
    for (const p of payloads) {
      ...
      for (const x of c.group)
        if (!combined.group.find((y) => y.id === x.id)) combined.group.push(x);
      for (const x of c.individual)
        if (!combined.individual.find((y) => y.id === x.id)) combined.individual.push(x);
      for (const x of c.personal)
        if (!combined.personal.find((y) => y.id === x.id)) combined.personal.push(x);
      for (const x of c.personal_events)
        if (!combined.personal_events.find((y) => y.id === x.id)) combined.personal_events.push(x);
    }
    const hasAny =
      combined.group.length > 0 ||
      combined.individual.length > 0 ||
      combined.personal.length > 0 ||
      combined.personal_events.length > 0;
    if (hasAny) {
  ...
  const handleAddRescheduleEvent = async (ev: ScheduleConflictPersonalEvent) => {
    ...
    try {
      await updatePersonalEvent(ev.id, {
        date: addEventReschedule.date,
        start_time: addEventReschedule.start,
  ...
  const handleAddDeleteBlocksAndSave = async () => {
    ...
    try {
      ...
      for (const p of addConflictModal.payloads) {
        created.push(await createScheduleSlot(slug, fmt.id, p));
      }
      setSlots((prev) => [...prev, ...created]);
  ...
  const handleRescheduleRescheduleEvent = async (ev: ScheduleConflictPersonalEvent) => {
    ...
    try {
      await updatePersonalEvent(ev.id, {
        date: rescheduleEventReschedule.date,
        start_time: rescheduleEventReschedule.start,
  ...
  const handleConflictDelete = async (ev: SlotPersonalConflict) => {
    ...
    try {
      await deletePersonalEvent(ev.id);
      setAssignConflict((prev) =>
        prev ? { ...prev, pending: prev.pending.filter((p) => p.id !== ev.id) } : null,
      );
    } finally {
  ...
  const handleConflictReschedule = async (
    ...
    try {
      ...
      const items = ((e.fields as Record<string, unknown>)?.conflicts ??
        []) as RescheduleConflictItem[];
    ...
    <div style={CARD}>
      ...
      >
        <div>
          ...
          >
            {t("bookedCount", { count: booked })} &middot;{" "}
            {t("availableCount", { count: available })}
          </p>
        </div>
        {!formOpen && (
      ...
      {addOpen && (
        <div
          style={{
            borderTop: slots.length > 0 ? "1px solid var(--color-border-light)" : undefined,
            paddingTop: slots.length > 0 ? 14 : 0,
          }}
        >
          <AddSlotForm
            onAdd={handleAdd}
      ...
      {addConflictModal &&
        (() => {
          ...
          const CROW: React.CSSProperties = {
            fontFamily: "var(--font-base)",
            fontSize: "clamp(12px, 0.78vw, 13px)",
            color: "var(--color-text-primary)",
            padding: "8px 0",
          ...
          const CNOTE: React.CSSProperties = {
            fontFamily: "var(--font-base)",
          ...
          return (
            <ModalShell
              onClose={() =>
                !addModalSaving &&
                !addEventLoading &&
                (setAddConflictModal(null), setAddPendingEvents([]), setAddEventReschedule(null))
              }
            ...
            >
              ...
              >
                  ...
                  <div>
                    ...
                    {info.personal.map((b) => (
                      <div key={b.id} style={CROW}>
                        {b.reason || t("personalBlockFallback")}&nbsp;
                        <span style={CMUTED}>
                  ...
                  <div>
                    ...
                    {addPendingEvents.map((ev) => {
                      ...
                      return (
                        <div
                          key={ev.id}
                          style={{ ...CROW, display: "flex", flexDirection: "column", gap: 6 }}
                        >
                          <div
                            style={{
                              display: "flex",
                          ...
                          >
                            <span>
                              ...
                              &nbsp;
                              <span style={CMUTED}>
                                {ev.date} &middot; {ev.start_time}&ndash;{ev.end_time}
                              </span>
                              {!ev.is_owner && (
                                <span style={{ ...CMUTED, marginLeft: 6 }}>{t("invited")}</span>
                              )}
                            </span>
                              ...
                              {ev.is_owner ? (
                                <>
                                  ...
                                  <button
                                    type="button"
                                    disabled={isLoading || !!addEventLoading}
                                    onClick={() => handleAddDeleteEvent(ev)}
                                    style={CBTN(true)}
                                  >
                                    {isLoading ? "…" : t("delete")}
                                  </button>
                                ...
                                <button
                                  type="button"
                                  disabled={isLoading || !!addEventLoading}
                                  onClick={() => handleAddDeclineEvent(ev)}
                                  style={CBTN(true)}
                                >
                            ...
                            >
                              <div>
                                <DatePicker
                                  size="sm"
                                  label={t("newDate")}
                                  value={addEventReschedule.date}
                                  onChange={(value) =>
                                    setAddEventReschedule((s) => (s ? { ...s, date: value } : s))
                                  }
                                />
                              </div>
                              <div>
                                ...
                                <TimePicker
                                  value={addEventReschedule.start}
                                  onChange={(v) =>
                                    setAddEventReschedule((s) => (s ? { ...s, start: v } : s))
                                  }
                                />
                              </div>
                              <div>
                                ...
                                <TimePicker
                                  value={addEventReschedule.end}
                                  onChange={(v) =>
                                    setAddEventReschedule((s) => (s ? { ...s, end: v } : s))
                                  }
                                />
                              </div>
                              <button
                ...
                {hasSessionConflicts && !blocked && (
                  <p style={{ ...CNOTE, color: "var(--color-danger)" }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {hasPersonalBlocks && (
                    <button
                      type="button"
                      onClick={handleAddDeleteBlocksAndSave}
                      disabled={addModalSaving || hasPendingEvents}
                      style={{
                        ...SUBMIT_BTN,
                        background: "var(--color-pink-dark)",
                        opacity: addModalSaving || hasPendingEvents ? 0.5 : 1,
                        cursor: addModalSaving || hasPendingEvents ? "not-allowed" : "pointer",
                      }}
                    >
                  ...
                  {!blocked && !hasSessionConflicts && (
                    <button
                      type="button"
                      onClick={handleAddProceed}
                      disabled={addModalSaving}
                      style={{
                        ...SUBMIT_BTN,
                        opacity: addModalSaving ? 0.7 : 1,
                        cursor: addModalSaving ? "not-allowed" : "pointer",
                      }}
                    >
      ...
      {rescheduleConflictModal &&
        (() => {
          ...
          return (
            <ModalShell
              onClose={() =>
                !rescheduleModalSaving &&
                !rescheduleEventLoading &&
                (setRescheduleConflictModal(null),
              ...
              >
                            ...
                            <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                              {ev.is_owner ? (
                                <>
                                  <button
                                    type="button"
                                    disabled={isLoading || !!rescheduleEventLoading}
                                    onClick={() =>
                                      setRescheduleEventReschedule(
                                        isRescheduling
                                          ? null
                                          : {
                                              eventId: ev.id,
                                  ...
                                  <button
                                    type="button"
                                    disabled={isLoading || !!rescheduleEventLoading}
                                    onClick={() => handleRescheduleDeleteEvent(ev)}
                                    style={CBTN(true)}
                                  >
                                    {isLoading ? "…" : t("delete")}
                                  </button>
                                ...
                                <button
                                  type="button"
                                  disabled={isLoading || !!rescheduleEventLoading}
                                  onClick={() => handleRescheduleDeclineEvent(ev)}
                                  style={CBTN(true)}
                                >
                                  {isLoading ? "…" : t("declineInvite")}
                                </button>
                            ...
                            >
                              <div>
                                <DatePicker
                                  size="sm"
                                  label={t("newDate")}
                                  value={rescheduleEventReschedule.date}
                                  onChange={(value) =>
                                    setRescheduleEventReschedule((s) =>
                                      s ? { ...s, date: value } : s,
                                    )
                                  }
                                />
                              ...
                              <div>
                                <label style={CLABEL}>{t("start")}</label>
                                <TimePicker
                                  value={rescheduleEventReschedule.start}
                                  onChange={(v) =>
                                    setRescheduleEventReschedule((s) =>
                                      s ? { ...s, start: v } : s,
                                    )
                              ...
                              <div>
                                <label style={CLABEL}>{t("end")}</label>
                                <TimePicker
                                  value={rescheduleEventReschedule.end}
                                  onChange={(v) =>
                                    setRescheduleEventReschedule((s) => (s ? { ...s, end: v } : s))
                                  }
                                />
                              </div>
                              <button
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {hasPersonalBlocks && (
                    <button
                      type="button"
                      onClick={handleRescheduleDeleteBlocksAndSave}
                      disabled={rescheduleModalSaving || hasPendingEvents}
                      style={{
                        ...SUBMIT_BTN,
                        background: "var(--color-pink-dark)",
                        opacity: rescheduleModalSaving || hasPendingEvents ? 0.5 : 1,
                        cursor:
                          rescheduleModalSaving || hasPendingEvents ? "not-allowed" : "pointer",
                      }}
                    >
                  ...
                  {!blocked && !hasSessionConflicts && (
                    <button
                      type="button"
                      onClick={handleRescheduleProceed}
                      disabled={rescheduleModalSaving}
                      style={{
                        ...SUBMIT_BTN,
                        opacity: rescheduleModalSaving ? 0.7 : 1,
                        cursor: rescheduleModalSaving ? "not-allowed" : "pointer",
                      }}
                    >
      ...
      {assignConflict && (
        <ModalShell
          ...
          onClose={() =>
            !assignConflictLoading && (setAssignConflict(null), setConflictRescheduleState(null))
          }
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                      {!isRescheduling && (
                        <button
                          type="button"
                          disabled={!!assignConflictLoading}
                          onClick={() => {
                            setConflictRescheduleState({
                              eventId: ev.id,
                              date: ev.date,
                              start: ev.start_time,
                              end: ev.end_time,
                      ...
                      {ev.is_owner ? (
                        <button
                          type="button"
                          disabled={assignConflictLoading === ev.id}
                          ...
                          style={{
                            ...
                            padding: "4px 10px",
                            cursor: "pointer",
                            opacity: assignConflictLoading === ev.id ? 0.5 : 1,
                          }}
                        >
                    ...
                    >
                      <div>
                        <DatePicker
                          size="sm"
                          label={t("date")}
                          value={conflictRescheduleState.date}
                          onChange={(v) =>
                            setConflictRescheduleState((s) => (s ? { ...s, date: v } : s))
                          }
                        />
                      </div>
                      <div>
                        ...
                        <TimePicker
                          value={conflictRescheduleState.start}
                          onChange={(v) =>
                            setConflictRescheduleState((s) => (s ? { ...s, start: v } : s))
                          }
                        />
                      </div>
                      <div>
                        ...
                        <TimePicker
                          value={conflictRescheduleState.end}
                          onChange={(v) =>
                            setConflictRescheduleState((s) => (s ? { ...s, end: v } : s))
                          }
                        />
                      </div>
                      <div style={{ gridColumn: "1 / -1", display: "flex", gap: 8 }}>
                        <button
                          ...
                          onClick={() =>
                            handleConflictReschedule(
                              ev,
                              conflictRescheduleState.date,
                              conflictRescheduleState.start,
                              conflictRescheduleState.end,
                            ).then(() => setConflictRescheduleState(null))
                          }
                          style={{
                            fontFamily: "var(--font-base)",
                            fontWeight: 600,
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            setConflictRescheduleState(null);
                            setAssignConflictRescheduleErr(null);
                          }}
                          style={{
                            fontFamily: "var(--font-base)",
                      ...
                      {assignConflictRescheduleErr && (
                        ...
                        >
                          ...
                          >
                            {assignConflictRescheduleErr.message}
                          </p>
                          {assignConflictRescheduleErr.items.map((c, i) => (
                            ...
                            >
                              {c.type === "group"
                                ? t("conflictType.group")
                                : c.type === "individual"
                                  ? t("conflictType.individual")
                                  : t("conflictType.personal")}
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {assignConflict.pending.length === 0 && (
                <button
                  type="button"
                  onClick={() => {
                    doAssign(assignConflict.slotId, assignConflict.enrollmentId);
...
function AddUnavailabilityForm({
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    if (recurrence === "one_time" && !date) {
      setErr(t("errorDateRequired"));
      return;
    }
    if (recurrence === "date_range" && (!date || !dateTo)) {
      setErr(t("errorBothDatesRequired"));
      return;
    }
    if (recurrence === "date_range" && dateTo < date) {
      setErr(t("errorEndDateAfterStart"));
      return;
    }
    setSaving(true);
    setErr(null);
    ...
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button type="button" style={tabBtn(!allDay)} onClick={() => setAllDay(false)}>
            {t("specific")}
          </button>
          <button type="button" style={tabBtn(allDay)} onClick={() => setAllDay(true)}>
            {t("allDay")}
          </button>
        </div>
      </div>
