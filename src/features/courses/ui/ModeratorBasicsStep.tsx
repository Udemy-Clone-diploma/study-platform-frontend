  ...
  return (
    <div style={{ opacity: locked ? 0.6 : 1 }}>
      ...
      >
        ...
        >
          <ModeratorItemStatusBadge
            status={itemStatuses[fieldKey] ?? null}
            onClick={() => onItemStatusToggle(fieldKey)}
            locked={locked}
          />
        </div>
      </div>
...
export function ModeratorBasicsStep(props: StepProps) {
  ...
  const displayShortDesc =
    (!lockedKeys.has("field-short-description") && draft?.short_description) ||
    course?.short_description ||
    "—";
  const displayFullDesc =
    (!lockedKeys.has("field-full-description") && draft?.full_description) ||
    course?.full_description ||
    "—";
  const displayImage = (!lockedKeys.has("field-icon") && draft?.image) || course?.image || null;
  const displayCategory =
    !lockedKeys.has("field-category") && draft
      ? (draft.category?.name ?? "—")
  ...
  return (
    ...
    >
      ...
      <NoteActionBlock
        ...
        itemAction={basicsAction}
        onItemActionChange={onBasicsActionChange}
        onSave={() => {}}
        title={t("sectionActionTitle")}
      />
