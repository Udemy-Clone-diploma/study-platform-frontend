...
function getFieldValue(
  ...
  switch (key) {
    case "field-title":
      return detail?.title ?? record.course_title;
    case "field-short-description":
      return detail?.short_description ?? "—";
    case "field-full-description":
      return detail?.full_description ?? "—";
    case "field-icon":
      return "";
    case "field-category":
      return record.course_category || detail?.category?.name || "—";
    case "field-level": {
...
export function RejectionDetailModal({ record, onClose, onMoveToDraft }: Props) {
  ...
  useEffect(() => {
    if (record.course_slug)
      getCourseBySlug(record.course_slug)
        .then(setDetail)
        .catch(() => {});
  }, [record.course_slug]);
    ...
    <ModalShell onClose={onClose} title={record.course_title} width="clamp(480px, 60vw, 860px)">
      <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        {Object.keys(basicsStatuses).length > 0 && (
          <Section
            title={t("basics")}
            action={record.basics_action}
            comment={record.basics_comment}
          >
        ...
        {modules.length > 0 && (
          <Section
            title={t("content")}
            action={record.content_action}
            comment={record.content_comment}
          >
          ...
          >
            <button
              ...
              onClick={async () => {
                setMoving(true);
                try {
                  await onMoveToDraft(record.course_slug);
                  onClose();
                } finally {
                  setMoving(false);
                }
