    ...
    <div className="flex w-full shrink-0 flex-col items-center gap-5 px-4 pt-4 pb-4 lg:w-[22.14vw] lg:items-center lg:gap-[1.25vw] lg:self-start lg:px-[1.5vw] lg:pt-[4.17vw] lg:pb-[6.25vw]">
      ...
      {teacherRating && parseFloat(teacherRating) > 0 && (
        ...
        >
          <Star
            aria-hidden
            className="h-6 w-6 fill-(--color-gold) text-(--color-gold) lg:h-[1.25vw] lg:w-[1.25vw]"
          />
          ...
          {editing ? (
            <div className="flex w-[277px] max-w-full flex-col gap-4 lg:w-full lg:gap-[0.833vw]">
              {PROFILE_SOCIALS.map((s) => (
                <div
                  key={s.key}
                  className="flex h-[46px] items-center gap-2.5 rounded-full border px-5 lg:h-auto lg:gap-[0.625vw] lg:px-[1.042vw] lg:py-[0.417vw]"
                  style={{
                ...
                >
                  ...
                  <input
                    ...
                    style={{
                      fontFamily: "var(--font-base)",
                      fontWeight: 400,
                      fontSize: "clamp(16px, 0.938vw, 18px)",
      ...
      {editing ? (
        <AccentButton
          size="md"
          onClick={onCancel}
          className={`mt-3 lg:mt-[0.417vw] ${PILL_BTN_MOBILE_CLASS} !bg-[var(--color-text-secondary)] hover:!bg-[var(--color-cancel-hover)]`}
        >
          {t("cancelAndExit")}
        </AccentButton>
      ) : (
        <AccentButton
          size="md"
          onClick={onEdit}
          className={`mt-3 lg:mt-[0.417vw] ${PILL_BTN_MOBILE_CLASS}`}
        >
          {t("edit")}
        </AccentButton>
      )}
