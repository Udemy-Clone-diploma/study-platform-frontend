...
export function CourseScheduleCard({ cohort, modules_count, lessons_count }: Props) {
  ...
  return (
    <aside className="flex flex-col gap-10 rounded-xl bg-(--color-white-50) px-4 py-8 shadow-(--shadow-usp-glass) backdrop-blur-md">
      <h3 className="text-2xl font-semibold text-(--color-text-primary)">Schedule</h3>
      ...
      {startDate && (
        <div className="flex flex-wrap items-baseline gap-3 text-xl text-(--color-text-primary)">
...
function ScheduleRow({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <li className="flex items-center gap-2 text-xl text-(--color-text-primary)">
      <span
        className="flex h-8 w-8 flex-shrink-0 items-center justify-center text-(--color-text-primary) [&>svg]:h-8 [&>svg]:w-8"
        aria-hidden="true"
      >
        {icon}
      </span>
