      ...
      <div className="flex flex-col gap-4">
        {rows.map((row) => (
          <div key={row.date} className="flex items-center gap-3">
            <span className="w-12 shrink-0 text-xs text-(--color-text-secondary)">{row.label}</span>
            <div className="flex flex-1 flex-col gap-1.5" aria-label={`${row.reviewed} reviewed`}>
              <div
      ...
      <div className="flex flex-col gap-5">
        {data.map((item) => (
          <div key={item.key} className="flex items-center gap-3">
            <span className="w-28 shrink-0 text-xs text-(--color-text-primary)">{item.label}</span>
        ...
        <header className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-base text-(--color-text-secondary)">
              Overview of your moderation activity over the last 7 days
            </p>
          </div>
          <button
            type="button"
