...
function toStudentReview(
  review: TopReview,
  studiedCourse: (course: string) => string,
): StudentReview {
  return {
    id: review.id,
    text: review.text,
...
export async function StudentReviewsSection({ reviews }: Props) {
  ...
  const cards = reviews.map((review) =>
    toStudentReview(review, (course) => t("studiedCourse", { course })),
  );
    ...
    <section style={{ background: "var(--gradient-feedback)" }}>
      <SectionContainer>
        ...
        >
          ...
          <div
            style={{ display: "flex", flexDirection: "column", gap: "clamp(16px, 1.04vw, 15px)" }}
          >
              ...
              return (
                ...
                >
                  {pair.map((review, colIndex) => {
                    ...
                    return (
                      <StudentReviewCard key={review.id} review={review} className={flexClass} />
                    );
                  })}
                </div>
              );
